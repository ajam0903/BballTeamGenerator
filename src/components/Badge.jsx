...
export default React.memo(function Badge({
    ...
    const sizeConfig = {
        xs: { container: "w-5 h-5", iconSize: 16 },
...
export function BadgeCollection({ badges, maxDisplay = 3, size = "small" }) {
    ...
    return (
        <div className={`flex items-center ${size === 'xs' ? 'gap-0.5' : 'gap-1'} ml-1`}>
            ...
            {hasMore && (
                <div className={`
                    ${size === 'xs' ? 'w-3 h-3 text-[10px]' :
                        size === 'small' ? 'w-5 h-5 text-xs' : 'w-7 h-7 text-sm'} 
